...
class GoodReceiveNoteReturnController extends Controller
{
    public function index()
    {
        ...
        $returns = GoodsReceivedNoteReturn::with(['user', 'goodReceiveNote.goodReceiveNoteProducts.product', 'goodReceiveNoteReturnProducts.product'])->latest()->paginate(20);
        ...
        $goodReceiveNotes = GoodsReceivedNote::with(['goodReceiveNoteProducts.product'])->orderByDesc('id')->get()->toArray();
        $user = auth()->user(); 
        ...
        return Inertia::render('goodsReceivedNoteReturns/Index', compact('returns', 'goodReceiveNotes', 'user', 'availableProducts', 'measurementUnits'));
    }
        ...
        try {
            ...
            foreach ($validated['products'] as $p) {
                ...
                if (!empty($p['quantity']) && $p['quantity'] > 0) {
                    ...
                    if ($product) {
                        // ensure numeric
                        $quantity = is_numeric($p['quantity']) ? (float)$p['quantity'] : floatval($p['quantity']);
    ...
    public function update(Request $request, GrnReturn $grnReturn)
    {
        ...
        try {
            ...
            $goodReceiveNoteReturn->update([
                'goods_received_note_return_id' => $validated['goods_received_note_id'],
                'date' => $validated['date'],
                'user_id' => $validated['user_id'],
            ]);
            ...
            foreach ($validated['products'] as $p) {
                ...
                if (!empty($p['quantity']) && $p['quantity'] > 0) {
                    ProductMovement::record($p['product_id'], ProductMovement::TYPE_GRN_RETURN, $p['quantity'], 'Good Receive Note Return #' . $goodReceiveNoteReturn->id);
                    $product = Product::find($p['product_id']);
                    if ($product) {
                        $quantity = is_numeric($p['quantity']) ? (float)$p['quantity'] : floatval($p['quantity']);
                        $purchaseToTransfer = is_numeric($product->purchase_to_transfer_rate) && $product->purchase_to_transfer_rate > 0 ? (float)$product->purchase_to_transfer_rate : 1.0;
    ...
    public function destroy(GoodReceiveNoteReturn $goodReceiveNoteReturn)
    {
        DB::beginTransaction();
        try {
