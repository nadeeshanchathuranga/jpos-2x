...
class StoreInventoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Get all products for filter dropdown with their units
        $products = Product::with(['purchaseUnit', 'brand', 'category'])
            ->select('id', 'name', 'barcode', 'purchase_unit_id', 'brand_id', 'category_id', 'store_quantity_in_purchase_unit')
            ->orderBy('name')
            ->get();
        ...
        return Inertia::render('StoreInventory/Index', [
            ...
            'products' => $products,
            'filters' => $request->only(['product_id', 'transaction_type', 'date_from', 'date_to', 'status']),
        ]);
    }
