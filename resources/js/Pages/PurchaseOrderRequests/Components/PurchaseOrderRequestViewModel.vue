<template>
    <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black bg-opacity-50">
        <div class="relative w-full max-w-4xl mx-4 my-8 bg-gray-50 rounded-2xl max-h-[90vh] overflow-y-auto shadow-xl">
            <!-- Header -->
            <div class="flex items-center justify-between p-6 border-b border-gray-200">
                <h2 class="text-2xl font-bold text-blue-600">👁️ Purchase Order Request Details</h2>
                ...
                <div class="mb-4 bg-white rounded-xl p-4 shadow-sm border border-gray-200">
                    ...
                    <div>
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div>
                                <label class="block mb-1 text-sm font-medium text-gray-500">Order Number</label>
                                <p class="text-base font-semibold text-gray-900">{{ por.order_number }}</p>
                            </div>
                            <div>
                                <label class="block mb-1 text-sm font-medium text-gray-500">Order Date</label>
                            ...
                            <table class="w-full text-left border-collapse">
                                ...
                                <tbody>
                                    <tr v-for="item in por.por_products" :key="item.id" class="border-b border-gray-200 hover:bg-gray-50 transition-colors duration-200">
                                        ...
                                        <td class="px-4 py-4 text-sm text-gray-900">{{ item.requested_quantity }}</td>
...
const getStatusClass = (status) => {
    ...
    return classes[status] || 'bg-gray-500 text-white px-4 py-1.5 rounded-[5px] font-medium text-xs';
};
