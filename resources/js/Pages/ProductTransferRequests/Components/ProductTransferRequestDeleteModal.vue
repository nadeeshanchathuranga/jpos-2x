<template>
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75">
    <div class="w-full max-w-md p-6 bg-gray-900 border-2 border-red-600 rounded-lg">
      ...
      <div v-if="ptr?.status !== 'pending'" class="mb-4 p-3 bg-red-900 border border-red-600 rounded text-red-200 text-sm">
        <p><strong>Error:</strong> Only pending PTRs can be deleted. Current status: <span class="font-semibold">{{ ptr?.status.toUpperCase() }}</span></p>
      </div>
      ...
      <div class="flex justify-end space-x-4">
        ...
        <button
          @click="confirmDelete"
          :disabled="isDeleting || ptr?.status !== 'pending'"
...
const confirmDelete = () => {
  if (!props.ptr || props.ptr.status !== 'pending') return;
  
  isDeleting.value = true;
  router.delete(`/ptr/${props.ptr.id}`, {
    onSuccess: () => {
