    ...
    <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black bg-opacity-75">
        <div class="relative w-full max-w-4xl p-6 mx-4 my-8 bg-black border-4 border-blue-600 rounded-lg max-h-[90vh] overflow-y-auto">
            ...
            <div class="flex items-center justify-between mb-6">
                <h2 class="text-2xl font-bold text-white">Stock Transfer Return Details</h2>
                ...
                <div class="mb-6 overflow-hidden border-2 border-blue-500 rounded-lg">
                    ...
                    <div class="p-6 bg-gray-900">
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div>
                                <label class="block mb-1 text-sm font-medium text-gray-400">Return Number</label>
                                <p class="text-lg font-semibold text-white">{{ stockTransferReturn.return_no }}</p>
                            </div>
                            <div>
                                <label class="block mb-1 text-sm font-medium text-gray-400">Return Date</label>
                                <p class="text-lg font-semibold text-white">{{ formatDate(stockTransferReturn.return_date) }}</p>
                            </div>
                            <div>
                                <label class="block mb-1 text-sm font-medium text-gray-400">User</label>
                                <p class="text-lg font-semibold text-white">{{ stockTransferReturn.user?.name || 'N/A' }}</p>
                            </div>
                            <div>
                            ...
                            <div class="md:col-span-2">
                                <label class="block mb-1 text-sm font-medium text-gray-400">Reason</label>
                                <p class="text-lg font-semibold text-white">{{ stockTransferReturn.reason || 'N/A' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="p-6 bg-gray-900">
                        <div class="overflow-x-auto">
                            <table class="w-full text-left text-white">
                                ...
                                <tbody>
                                    <tr v-for="item in stockTransferReturn.products" :key="item.id" class="border-b border-gray-700">
                                        <td class="px-4 py-3">{{ item.product?.name || 'N/A' }}</td>
                                        <td class="px-4 py-3">{{ item.stock_transfer_quantity }}</td>
                                        <td class="px-4 py-3">{{ item.measurement_unit?.name || 'N/A' }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
