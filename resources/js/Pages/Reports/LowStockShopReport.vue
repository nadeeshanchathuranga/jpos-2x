      ...
      <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <div class="flex items-center gap-4 mb-2">
              ...
              <h1 class="text-3xl font-bold text-white">🏪 Shop Low Stock Report</h1>
            </div>
            <p class="text-gray-400">Products in shop that are at or below configured low-stock margins.</p>
          </div>
        ...
        <div class="bg-gray-800 rounded-lg p-6 shadow-lg mb-6">
          <div class="flex justify-between items-center mb-4">
            <h3 class="text-xl font-semibold text-white">Shop Low Stock Details</h3>
          ...
          <div v-if="products.length === 0" class="text-center text-gray-400 py-8">No low stock products found in shop.</div>
        </div>
      </div>
    </div>
  </AuthenticatedLayout>
...
const exportPdfUrl = computed(() => route('reports.export.low-stock-shop.pdf', { start_date: startDate.value, end_date: endDate.value }));
const exportCsvUrl = computed(() => route('reports.export.low-stock-shop.csv', { start_date: startDate.value, end_date: endDate.value }));

const exportPdf = async () => {
  await logActivity('create', 'low_stock_shop_report', { action: 'export_pdf', total: props.products.length, start_date: startDate.value, end_date: endDate.value });
  window.location.href = exportPdfUrl.value;
};

const exportCsv = async () => {
  await logActivity('create', 'low_stock_shop_report', { action: 'export_csv', total: props.products.length, start_date: startDate.value, end_date: endDate.value });
...
const filterReports = () => {
  ...
  router.get(route('reports.low-stock-shop'), params, { preserveState: true, preserveScroll: true });
};

const resetFilter = () => {
  startDate.value = '';
  endDate.value = '';
  router.get(route('reports.low-stock-shop'), {}, { preserveState: false, preserveScroll: false });
};
</script>
